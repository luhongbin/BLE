    ...
    private void parseAccessMessage(final AccessMessage message) {
        ...
        switch (opCodeLength) {
            ...
            case 1:
                ...
                if (message.getOpCode() == ApplicationMessageOpCodes.SENSOR_DESC_STATUS) {
                    final SensorDescStatus status = new SensorDescStatus(message);
                    mInternalTransportCallbacks.updateMeshNetwork(status);
                    mMeshStatusCallbacks.onMeshMessageReceived(message.getSrc(), status);
                } else
                if (message.getOpCode() == ApplicationMessageOpCodes.SENSOR_STATUS) {
